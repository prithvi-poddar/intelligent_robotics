...
class A_star():
    def __init__(self, n_row=10, n_col=10, start=[9,0], goal=[0,9], obstacles=[]):
        self.n_row = n_row
        self.n_col = n_col
        self.start = start
        self.goal = goal
    ...
    def get_cost(self):
        cost = 0
        for i in range(len(self.back_y)-1):
            cost += np.sqrt((self.back_x[i]-self.back_x[i+1])**2 + (self.back_y[i]-self.back_y[i+1])**2)
        return cost
